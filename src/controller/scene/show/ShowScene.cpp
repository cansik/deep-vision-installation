...
void ShowScene::setup() {
    ...
    //setupNatureShow();
    setupShortShow();
    this->animation = new Animation<LIGHT_SIZE>(keyPoints, installation->getSettings()->getShowSpeed());
...
void ShowScene::loop() {
    ...
    for (auto i = 0; i < installation->getSize(); i++) {
        ...
        // update color
        portal->getLed()->setBrightness(values[i].r, false);
    }
...
void ShowScene::addRandomFlashTween(float time, float flashTime, float startProbability, float endProbability,
    ...
    for (int i = 0; i < discreteTime; i++) {
        ...
        auto keyPointSet = KeyPointSet<LIGHT_SIZE>(flashTime, baseColor);

        // flash if probability matches
        for (int r = 0; r < LIGHT_SIZE; r++) {
...
void ShowScene::addShiftTween(float time, RGBColor startColor, RGBColor endColor,
    ...
    auto indexes = randomizePhase ? MathUtils::getRandomIndexes(0, LIGHT_SIZE, LIGHT_SIZE)
                                  : MathUtils::getIndexes(0, LIGHT_SIZE);
